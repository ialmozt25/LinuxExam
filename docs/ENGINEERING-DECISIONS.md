...
## DECISION-013 (2026-09-22)

**Контекст:** после миграции на семантические токены оставались три проблемы
контраста по WCAG 2.2 AA (порог 4.5:1 для текста):

- `correct` `#4CAF50` на светлой карточке `#FFFFFF` — 2.78:1;
- `wrong` `#F44336` на светлой карточке — 3.68:1;
- `wrong` `#F44336` на тёмной карточке `#2D2D2D` — 3.74:1.

Провалы предсуществующие: `COLORS.correct`/`wrong` и `--success`/`--danger` были
одними и теми же hex, поэтому нечитаемость существовала до миграции и просто
стала заметной в светлой теме.

**Решение:** `--success`/`--danger` сделаны тематическими.

| Токен | light | dark |
|---|---|---|
| `--theme-success` | `#377E3A` | `#4CAF50` |
| `--theme-danger` | `#CF392E` | `#FF5C4A` |

Значения подобраны так, чтобы проходить 4.5:1 на **обоих** фонах темы — и на
карточке (`--bg-surface`), и на странице (`--bg-primary`). Из `:root` статические
`--success`/`--danger` удалены; маппинг добавлен в `[data-theme-source="inherit"]`
и `[data-theme-source="manual"]`. Результат: light 4.99/4.91, dark 4.95/4.51.

`--accent` и `--warning` не тронуты. `--accent` остаётся Telegram-aware: внутри
клиента цвет приходит от Telegram, и подменять его нельзя.

**Последствия:**

- `COLORS` из `colors.ts` больше не используется нигде; файл удалён.
- Barrel `src/presentation/theme/index.ts` больше не реэкспортирует `./colors` —
  **barrel изменён: убран экспорт `COLORS`**.
- Мёртвый экспорт `CSS_VARS` (0 использований) удалён вместе с файлом.
- `--warning` (`#FF9800`) остаётся фиксированным — вне scope этой задачи.

**Сознательно НЕ исправлено (отдельные задачи):**

- **Белый текст на `--accent`** = 3.12:1 при необходимости 4.5:1 для текста.
  Это ограничение: внутри Telegram цвет кнопки задаёт клиент, и заменить его
  нельзя, не сломав следование теме. Вне Telegram можно было бы затемнить
  фолбэк. Влияет на все accent-кнопки («Продолжить», «Заметить», CTA и т.д.).
- Пара «белый текст на `--success`/`--danger`» в тёмной теме даёт 2.78/3.05:1.
  В коде такой комбинации нет: `--success`/`--danger` используются только как
  цвет текста, граница и градиент, никогда как фон под белым текстом. Если
  когда-нибудь понадобится заливка этими цветами — потребуется отдельный
  «text-on-status» токен.

## Политика проверок (2026-09-26)

**Контекст:** у промптов агентов и у чат-ревью разная стоимость ревью.

**Решение:** дополнительные 5-проходные проверки применяются к промпту ТОЛЬКО по
явному запросу капитана в чате. Дефолт — 1–2 прохода. Число проходов внутри
промпта агента задаётся самим промптом; дополнительные проверки в чате на бюджет
агента не влияют.

**Обоснование:** 5-проходный adversarial-протокол — дорогой инструмент под
конкретный риск; применение «по умолчанию» тратит бюджет там, где хватает 1–2
проходов. При этом проходы внутри промпта и проходы в чате — разные бюджеты:
чат-ревью не расходует контекст агента, поэтому его объём не меняет поведение
промпта.

## DECISION-014 (2026-09-26)

**Контекст:** закрытие milestone M3 «Центр Управления» (дашборд состояния проекта).
**Решение:** M3 закрыт; доставлены 6 подшагов — M3.1 (`.project/state.json`), M3.2 (`tools/gen-state.mjs`),
M3.3 (`docs/dashboard/dashboard.css`), M3.4 (`docs/dashboard/dashboard.html` + `dashboard.js`),
M3.5 (auto-sync `state.json` → `docs/dashboard/`), M3.6 (закрытие: STATE, DECISIONS, sync).
**Обоснование:** все артефакты M3 в origin/main; гейты зелёные (qc 166/0, typecheck, vitest 137/137,
shuffle 166); дашборд получает актуальный `state.json` автоматически, без ручного копирования.
**Открыто:** M3.7 «Центр разработки — переиспользование» — следующий milestone (planned).
