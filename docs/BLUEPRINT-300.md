...
|---|---|---|---|---|
| `file_permissions` | available | 12 | 20 | +8 |
| `file_management` | available | 12 | 22 | +10 |
| `process_management` | available | 11 | 18 | +7 |
| `essential_tools` | available | 7 | 15 | +8 |
...
| `users_groups` | available | 12 | 19 | +7 |
| `security` | available | 12 | 30 | +18 |
| **итого** | — | **66** | **300** | **+234** |

Проверка суммы: planned-темы 8 × 22 = **176**; расширение available +8 +10 +7 +8 +7 +18 = **58** (доступные 66 → 124); 124 + 176 = **300** (потолок 320 — резерв на перегенерацию отбракованных слотов, не на расширение списка тем).
...
- **Сумма доменов и сумма тем — разные оси.** По темам план даёт 300 (доступные 124 + плановые 176); по `objective_domain` распределение изменится только после генерации, потому что домен задаётся на уровне вопроса, а не темы.
...
| 9 | `security` | available | +18 (до 30) | расширение после всех новых тем; SELinux/firewalld/sudo уже покрыты 12 вопросами |
| 10 | `file_management` | available | +10 (до 22) | расширение существующих сабтопиков |
| 11 | `file_permissions` | available | +8 (до 20) | расширение; тема самая насыщенная (12 вопросов, домен 9) |
| 12 | `process_management` | available | +7 (до 18) | расширение |
| 13 | `essential_tools` | available | +8 (до 15) | расширение |
| 14 | `users_groups` | available | +7 (до 19) | расширение |

Итого по плану: 8 батчей × 22 (176) + 58 расширения = **234 новых вопроса**, 66 + 234 = **300** (доступные 66 → 124, плановые 0 → 176).
...
## Принятые решения (2026-09-24)

1. **Формат файлов вопросов:** 14 отдельных JSON по темам
   (`src/data/questions/{topic}.json`), lazy-load по теме.
   Финализация — в Проходе 3 (lazy-load).
2. **Порог Haladyna:** оставить 8/10. Качество — non-negotiable
   при цели 300+.
3. **Cosine:** блокировать по темам. Внутри темы — полный O(n²);
   между темами — не сравнивать. Ожидаемый объём: ~3 200 пар
   на 300 вопросов вместо 44 850. Переписать `tools/cosine.cjs`
   в topic-batched режим (Проход 3).
4. **Bundle:** lazy-load обязателен ДО первого батча. 138.77 kB
   уже за watch-порогом 137 kB.
5. **Cosine-модель:** отложить. Jaccard 0.9 — primary defense,
   cosine вторичен. Русскоязычная модель — после 300.
6. **Domain mapping (3–9):** отдельная research-сессия. Не блокер
   для первого батча. Долг.
7. **retired.json:** отложить. Бэклог.
8. **Стоп-правило аудита:** скорректировать. Было: «> 2 батча
   только minor → стоп». Стало: «> 3 батча подряд только minor →
   consensus-only на следующий батч, НЕ стоп».
9. **Дубль `chage -M` (ug_009 / sec_011):** бэклог. Cleanup-батч
   после 300.
10. **Вариант разрыва 26:** выбран (a) — перераспределение +26
   по available. Новые цели: fp 20, fm 22, pm 18, et 15,
   ug 19, sec 30. Итог 300.
